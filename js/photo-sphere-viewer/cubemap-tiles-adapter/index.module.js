/*!
 * Photo Sphere Viewer / Cubemap Tiles Adapter 5.14.1
 * @copyright 2015-2026 Damien "Mistic" Sorel
 * @licence MIT (https://opensource.org/licenses/MIT)
 */

// src/CubemapTilesAdapter.ts
import { AbstractAdapter, CONSTANTS, PSVError as PSVError2, events, utils as utils3 } from "@photo-sphere-viewer/core";
  ...
  if (isMultiTiles(panorama)) {
    panorama.levels.forEach((level) => {
      ...
      if ("zoomRange" in level) {
        utils2.logWarn('CubemapTilesAdapter: "zoomRange" property is deprecated and must be removed');
      }
    });
...
var _CubemapTilesAdapter = class _CubemapTilesAdapter extends AbstractAdapter {
  constructor(viewer, config) {
    ...
    if (!CubemapAdapter) {
      throw new PSVError2("CubemapTilesAdapter requires CubemapAdapter");
    }
  ...
  static withConfig(config) {
    return [_CubemapTilesAdapter, config];
  }
  init() {
    super.init();
...
_CubemapTilesAdapter.id = "cubemap-tiles";
_CubemapTilesAdapter.VERSION = "5.14.1";
_CubemapTilesAdapter.supportsDownload = false;
var CubemapTilesAdapter = _CubemapTilesAdapter;
export {
  CubemapTilesAdapter
};
