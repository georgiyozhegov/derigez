  ...
  if (isMultiTiles(panorama)) {
    panorama.levels.forEach((level) => {
      ...
      if ("zoomRange" in level) {
        import_core2.utils.logWarn('CubemapTilesAdapter: "zoomRange" property is deprecated and must be removed');
      }
    });
...
var _CubemapTilesAdapter = class _CubemapTilesAdapter extends import_core3.AbstractAdapter {
